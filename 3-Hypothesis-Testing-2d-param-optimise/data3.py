# -*- coding: utf-8 -*-
"""
Created on Mon Nov 13 19:42:14 2017
...
N = 100
url = "http://www.sr.bham.ac.uk/~imandel/Inference/data_100hr.txt"
im = ur.urlopen(url)
data = np.zeros(N)
...
fig.savefig("Q3.3-M1.pdf", format = 'pdf')
plt.show
...
ax.set_ylim(bottom=0)
fig.savefig("Q3.3-M2-A0.pdf", format = 'pdf')
plt.show
...
ax.set_ylim(bottom=0)
fig.savefig("Q3.3-M2-Tau.pdf", format = 'pdf')
plt.show
...
plt.xlabel('Initial Amplitude, A0 (arb)')
fig.savefig("Q3.3-M2-Tau&A0.pdf", format = 'pdf')
plt.show
...
#Plot largest data set against models with bets fit models
M1 = []
M21 = []
M22 = []
for item in t:
    M1.append(B + 1.1)
    M21.append(B + 1.2*np.exp(-item/38))
    M22.append(B + modeA0*np.exp(-item/modeTau))
fig = plt.figure()
ax = fig.add_subplot(111)
Data = ax.plot(t, data, label="Data")
Model0 = ax.plot(t, M1, label="Model 1: L0 = 1.1")
Model1 = ax.plot(t, M21, label="Model 2: A0 = 1.2, Tau = 38")
Model2 = ax.plot(t, M22, label="Model 2: A0 = 1.15, Tau = 33")
plt.title('Graph Showing 100 Hours of Data Against Best Fit Models', y=1.08)
plt.ylabel('Luminosity (arb)')
plt.xlabel('Time (Hours)')
plt.legend(loc=0)
fig.savefig("Q3.3-BestFit.pdf", format = 'pdf')
plt.show
